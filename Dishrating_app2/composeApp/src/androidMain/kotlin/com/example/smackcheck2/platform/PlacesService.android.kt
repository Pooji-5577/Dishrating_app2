    ...
    actual suspend fun getPlaceDetails(placeId: String): NearbyRestaurant? = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            val url = "https://maps.googleapis.com/maps/api/place/details/json" +
                    "?place_id=$placeId" +
                    "&fields=place_id,name,vicinity,formatted_address,geometry,rating,user_ratings_total,price_level,photos,opening_hours" +
                    "&key=$apiKey"

            val response: PlaceDetailsResponse = httpClient.get(url).body()

            Log.d(TAG, "Place Details API status: ${response.status}")
            if (response.status != "OK") {
                Log.e(TAG, "Place Details API error: ${response.errorMessage ?: response.status}")
                return@withContext null
            }

            val place = response.result ?: run {
                Log.e(TAG, "Place Details API returned no result")
                return@withContext null
            }

            Log.d(TAG, "Successfully fetched place details for: ${place.name}")
            ...
            Log.e(TAG, "Error getting place details for $placeId", e)
            null
        }
    }
}
