...
class DatabaseRepository {
     ...
     */
    private fun generateId(prefix: String = "id"): String {
    ...
    suspend fun createOrGetRestaurant(restaurant: Restaurant): Result<Restaurant> {
        return try {
            ...
            // Use upsert to handle explicit ID insertion
            val dto = RestaurantDto(
            ...
            val created = postgrest["restaurants"]
                ...
                .decodeSingle<RestaurantDto>()
            println("DatabaseRepository: ✓ Restaurant created successfully with ID: ${created.id}")
            Result.success(created.toRestaurant())
        } catch (e: Exception) {
    ...
    suspend fun toggleLike(userId: String, ratingId: String): Result<Boolean> {
        return try {
            ...
            if (existing != null) {
                ...
                // Like
                val like = LikeDto(id = generateId("like"), userId = userId, ratingId = ratingId)
